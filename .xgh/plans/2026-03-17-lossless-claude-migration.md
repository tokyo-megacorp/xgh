...
**Goal:** Replace all Cipher MCP tool references across xgh with `@ipedro/lossless-claude` MCP tools, preserving memory semantics via a complete Cipher → lossless-claude mapping.
...
**Tech Stack:** `@ipedro/lossless-claude` MCP server (stdio), lossless-claude daemon (HTTP :3737), SQLite episodic layer + Qdrant semantic layer. No new code — all changes are prose substitutions in Markdown instruction files.
...
Replaces @byterover/cipher with @ipedro/lossless-claude across:
