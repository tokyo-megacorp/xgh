...
3. The dispatcher orchestrates the workflow:
   ...
   - Stores all inter-agent messages in Cipher workspace under the `thread_id`
...
The collaboration-dispatcher agent (`agents/collaboration-dispatcher.md`) manages the full workflow lifecycle — reading templates, assigning roles, dispatching tasks, and monitoring progress via Cipher workspace messages.
