...
Expected: FAIL — `.gitignore` missing `model-profiles.json`, router skill doesn't exist yet.

- [ ] **Step 3: Add model-profiles.json to .gitignore**

Add this line to `.gitignore` after the `.xgh/local/` entry:

```
.xgh/model-profiles.json
```
...
git commit -m "test: add model profiles test, gitignore model-profiles.json"
```
...
Read `.xgh/model-profiles.json` if it exists. This file is auto-generated by the curate step of dispatch skills.
...
After the dispatch completes, append one observation to `.xgh/model-profiles.json`. Create the file if it doesn't exist.

```yaml
# Append to .xgh/model-profiles.json
- agent: gemini
  model: <the -m flag value, or "default" if none was passed>
  effort: <the --effort value, or "default" if none was passed>
  archetype: <set by router if dispatched via /xgh-dispatch, otherwise "unknown">
...
Confirm `.gitignore` contains `.xgh/model-profiles.json`.
