...
- The installer registers them via `register_plugin()` (lines 272-403), copying to `~/.claude/plugins/cache/tokyo-megacorp/xgh/<version>/skills/` (confirmed: 23 skills present there too).
...
- `~/.claude/plugins/cache/tokyo-megacorp/xgh/<version>/skills/` (registered for Claude Code)

**Fix needed:** Update the doctor skill (`plugin/skills/doctor/doctor.md`) to check the correct path: `~/.xgh/pack/plugin/skills/` or `~/.claude/plugins/cache/tokyo-megacorp/xgh/*/skills/`.
...
- Skills check: look at `~/.claude/plugins/cache/tokyo-megacorp/xgh/*/skills/` instead of `~/.xgh/pack/skills/`
