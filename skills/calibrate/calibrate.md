---
name: xgh:calibrate
description: "Use when running /xgh-calibrate or asking to tune the dedup threshold. Calibrates similarity threshold against real memory pairs, computes F1 scores at multiple thresholds, and offers to update analyzer.dedup_threshold in ingest.yaml."
---
...
1. **Sample pairs**: Use [SEARCH] with diverse queries to gather N memories through the available/native memory mechanism (configurable via `calibration.sample_size`, default 50). Form random pairs from the results.
